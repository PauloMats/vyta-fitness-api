...
async function main() {
  ...
  const sampleMessage = await prisma.directMessage.create({
    data: {
      senderId: luana.id,
      recipientId: trainerOne.id,
      trainerId: trainerOne.id,
      studentId: luana.id,
      category: MessageCategory.TRAINING,
      subject: 'Dúvida sobre o treino',
      body: 'Prof, no terra romeno devo manter o joelho semi-flexionado do início ao fim?',
    },
  });

  const sampleSupportTicket = await prisma.supportTicket.create({
    data: {
      userId: trainerOne.id,
      category: SupportCategory.QUESTIONS,
      subject: 'Dúvida sobre cadastro de aluno',
      message: 'Preciso importar um aluno de outra plataforma. Qual o fluxo recomendado?',
  ...
  await prisma.notification.createMany({
    data: [
      {
        recipientId: luana.id,
        type: NotificationType.WORKOUT,
        title: 'Treino de hoje',
        body: 'Seu treino Upper Body está programado para hoje às 19h.',
        actionHref: '/workouts/today',
      ...
      },
      {
        ...
        body: 'Luana recebeu um comentário no post vinculado ao treino.',
        meta: { postId: postOne.id },
      },
      {
        recipientId: caio.id,
        type: NotificationType.SYSTEM,
        title: 'Meta atualizada',
        body: 'Seu plano Corrida 10K + Força recebeu novos ajustes.',
        meta: { workoutPlanId: planTwo.id },
      },
      {
        recipientId: trainerOne.id,
        senderId: luana.id,
        directMessageId: sampleMessage.id,
        type: NotificationType.MESSAGE,
        title: 'Nova mensagem de aluno',
        body: 'Prof, no terra romeno devo manter o joelho semi-flexionado do início ao fim?',
        actionHref: `/students?studentId=${luana.id}&compose=1`,
        actionLabel: 'Abrir mensagem',
        meta: { directMessageId: sampleMessage.id, studentId: luana.id },
      },
      {
        recipientId: pedro.id,
        senderId: trainerTwo.id,
        type: NotificationType.MESSAGE,
        title: 'Solicitação em análise',
        body: 'Bruno Lima está avaliando sua solicitação de acompanhamento.',
      },
      {
        recipientId: bia.id,
        senderId: caio.id,
        ...
        body: 'Caio Menezes enviou uma solicitação de amizade.',
      },
      {
        recipientId: admin.id,
        senderId: trainerOne.id,
        supportTicketId: sampleSupportTicket.id,
        type: NotificationType.SUPPORT,
        title: 'Novo ticket de suporte',
        body: 'Preciso importar um aluno de outra plataforma. Qual o fluxo recomendado?',
        actionHref: '/support',
        actionLabel: 'Abrir ticket',
        meta: { supportTicketId: sampleSupportTicket.id },
      },
    ],
  });
